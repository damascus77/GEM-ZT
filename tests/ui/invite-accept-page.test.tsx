  ...
  it('shows org + role and a form when the token previews as valid', async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        ...
        return new Response(JSON.stringify({ org: { name: 'Acme Co' }, role: 'admin' }), {
          status: 200,
        });
      })
    );
  ...
  it('submitting the form POSTs to the accept endpoint and redirects on success', async () => {
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      if (url === `/api/v1/invitations/${TOKEN}`) {
        return new Response(JSON.stringify({ org: { name: 'Acme Co' }, role: 'editor' }), {
          status: 200,
        });
      }
      if (url === `/api/v1/invitations/${TOKEN}/accept` && init?.method === 'POST') {
        return new Response(JSON.stringify({ user: { id: 'u1', username: 'newuser' } }), {
          status: 201,
        });
      }
    ...
    await waitFor(() => {
      const post = fetchMock.mock.calls.find(
        ([u, i]) => u === `/api/v1/invitations/${TOKEN}/accept` && i?.method === 'POST'
      );
